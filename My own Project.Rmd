...
mlm_summary_out <- function(model) {
  ...
  CI = lme4::confint.merMod(model, method = "boot", nsim = 10) # increase to 500 for serious run! 
...
dtLong <- dtLong %>%
  ...
  mutate(
    ...
    negEmoCountry_sd = sd(negEmo, na.rm = T),
    posEmoCountry_mean = mean(posEmo, na.rm = T),
    posEmoCoutry_sd = sd(posEmo, na.rm = T),
    PFS01Country_mean = mean(PFS01, na.rm = T),
    PFS01Country_sd = sd(PFS01, na.rm = T),
    fail01Country_mean = mean(fail01, na.rm = T),
    fail01Country_sd = sd(fail01, na.rm = T),
    happyCountry_mean = mean(happy, na.rm = T),
    happyCountry_sd = sd(happy, na.rm = T),
    c19ProSo03Country_mean = mean(c19ProSo03, na.rm = T),
    c19ProSo03Country_sd = sd(c19ProSo03, na.rm = T),
  ) %>%
  ungroup()
...
```{r}
ana_lv1 <- lme4::lmer(WB ~ negEmo_cw + posEmo_cw + PFS_cw + disemp_cw +
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
```{r}
ana_lv1_int <- lme4::lmer(WB ~ negEmo_cw*disemp_cw + posEmo_cw*disemp_cw + PFS_cw*disemp_cw +
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
```{r}
ana_lv12 <- lme4::lmer(WB ~ negEmo_cw + posEmo_cw + PFS_cw + disemp_cw + 
                         negEmoPP_cw + posEmoPP_cw + PFSPP_cw + disempPP_cw +
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
```{r}
ana_lv12_int <- lme4::lmer(WB ~ negEmo_cw*disemp_cw + posEmo_cw*disemp_cw + PFS_cw*disemp_cw + 
                             negEmoPP_cw*disempPP_cw + posEmoPP_cw*disempPP_cw + PFSPP_cw*disempPP_cw +
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
# 
# interactions::sim_slopes(ana_lv12_int, pred = "posEmo_cw", modx = "disemp_cw", johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
# interactions::sim_slopes(ana_lv12_int, pred = "negEmo_cw", modx = "disemp_cw", johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
# interactions::sim_slopes(ana_lv12_int, pred = "PFS_cw", modx = "disemp_cw", johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
# interactions::sim_slopes(ana_lv12_int, pred = "posEmoPP_cw", modx = "disempPP_cw", johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
# interactions::sim_slopes(ana_lv12_int, pred = "negEmoPP_cw", modx = "disempPP_cw", johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
# interactions::sim_slopes(ana_lv12_int, pred = "PFSPP_cw", modx = "disempPP_cw", johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)

```
#### level 3 predictors
If at all we are underestimating effects on level 2 due to potential small cluster size (Schunck, 2016)
```{r}
ana_lv123 <- lme4::lmer(WB ~ negEmo_cw + posEmo_cw + PFS_cw + disemp_cw + 
                          negEmoPP_cw + posEmoPP_cw + PFSPP_cw + disempPP_cw + 
                      negEmoCtry_cw + posEmoCtry_cw + PFSCtry_cw + disempCtry_cw +
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
```{r}
ana_lv123_int <- lme4::lmer(WB ~ negEmo_cw*disemp_cw + posEmo_cw*disemp_cw + PFS_cw*disemp_cw + 
                              negEmoPP_cw*disempPP_cw + posEmoPP_cw*disempPP_cw + PFSPP_cw*disempPP_cw +
                      negEmoCtry_cw*disempCtry_cw + posEmoCtry_cw*disempCtry_cw + PFSCtry_cw*disempCtry_cw +
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
```{r}
ana_lv123_intCrossAll <- lme4::lmer(WB ~ negEmo_cw*disemp_cw + posEmo_cw*disemp_cw + PFS_cw*disemp_cw + 
                             negEmoPP_cw*disempPP_cw + posEmoPP_cw*disempPP_cw + PFSPP_cw*disempPP_cw +  
                             negEmo_cw*disempPP_cw + posEmo_cw*disempPP_cw + PFS_cw*disempPP_cw +  
                             negEmoCtry_cw*disempCtry_cw + posEmoCtry_cw*disempCtry_cw + PFSCtry_cw*disempCtry_cw +
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)

ana_lv123_intCrossOnly <- lme4::lmer(WB ~ negEmo_cw + posEmo_cw + PFS_cw + disemp_cw +
                             negEmoPP_cw + posEmoPP_cw + PFSPP_cw +  disempPP_cw +
                             negEmo_cw*disempPP_cw + posEmo_cw*disempPP_cw + PFS_cw*disempPP_cw +  
                             negEmoCtry_cw + posEmoCtry_cw + PFSCtry_cw + disempCtry_cw +
                    (1|coded_country) + # each country can have its own intercept
...
tmp_posEmo <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "posEmo_cw", modx = "disemp_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_negEmo <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "negEmo_cw", modx = "disemp_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_PFS <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "PFS_cw", modx = "disemp_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_posEmoPP <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "posEmoPP_cw", modx = "disempPP_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_posEmoPP
tmp_negEmoPP <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "negEmoPP_cw", modx = "disempPP_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_PFSPP <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "PFSPP_cw", modx = "disempPP_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_PFSPP
tmp_posEmoCross <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "posEmo_cw", modx = "disempPP_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_negEmoCross <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "negEmo_cw", modx = "disempPP_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_negEmoCross
tmp_PFSCross <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "PFS_cw", modx = "disempPP_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_posEmoCtry <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "posEmoCtry_cw", modx = "disempCtry_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_negEmoCtry <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "negEmoCtry_cw", modx = "disempCtry_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_PFSCtry <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "PFSCtry_cw", modx = "disempCtry_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
```
...
```{r}
paths_WB <- data.frame(
  posEmo_plus1DisempSD_A = tmp_posEmo$slopes$Est.[1],
  posEmo_plus1DisempSD_varA = tmp_posEmo$slopes$S.E.[1],
  posEmo_minus1DisempSD_A = tmp_posEmo$slopes$Est.[3],
  posEmo_minus1DisempSD_varA = tmp_posEmo$slopes$S.E.[3],
  negEmo_plus1DisempSD_A = tmp_negEmo$slopes$Est.[1],
  negEmo_plus1DisempSD_varA = tmp_negEmo$slopes$S.E.[1],
  negEmo_minus1DisempSD_A = tmp_negEmo$slopes$Est.[3],
  negEmo_minus1DisempSD_varA = tmp_negEmo$slopes$S.E.[3],
  PFS_plus1DisempSD_A = tmp_PFS$slopes$Est.[1],
  PFS_plus1DisempSD_varA = tmp_PFS$slopes$S.E.[1],
  PFS_minus1DisempSD_A = tmp_PFS$slopes$Est.[3],
  PFS_minus1DisempSD_varA = tmp_PFS$slopes$S.E.[3],
  posEmoPP_plus1DisempSD_A = tmp_posEmoPP$slopes$Est.[1],
  posEmoPP_plus1DisempSD_varA = tmp_posEmoPP$slopes$S.E.[1],
  posEmoPP_minus1DisempSD_A = tmp_posEmoPP$slopes$Est.[3],
  posEmoPP_minus1DisempSD_varA = tmp_posEmoPP$slopes$S.E.[3],
  negEmoPP_plus1DisempSD_A = tmp_negEmoPP$slopes$Est.[1],
  negEmoPP_plus1DisempSD_varA = tmp_negEmoPP$slopes$S.E.[1],
  negEmoPP_minus1DisempSD_A = tmp_negEmoPP$slopes$Est.[3],
  negEmoPP_minus1DisempSD_varA = tmp_negEmoPP$slopes$S.E.[3],
  PFSPP_plus1DisempSD_A = tmp_PFSPP$slopes$Est.[1],
  PFSPP_plus1DisempSD_varA = tmp_PFSPP$slopes$S.E.[1],
  PFSPP_minus1DisempSD_A = tmp_PFSPP$slopes$Est.[3],
  PFSPP_minus1DisempSD_varA = tmp_PFSPP$slopes$S.E.[3],
  posEmoCross_plus1DisempSD_A = tmp_posEmoCross$slopes$Est.[1],
  posEmoCross_plus1DisempSD_varA = tmp_posEmoCross$slopes$S.E.[1],
  posEmoCross_minus1DisempSD_A = tmp_posEmoCross$slopes$Est.[3],
  posEmoCross_minus1DisempSD_varA = tmp_posEmoCross$slopes$S.E.[3],
  negEmoCross_plus1DisempSD_A = tmp_negEmoCross$slopes$Est.[1],
  negEmoCross_plus1DisempSD_varA = tmp_negEmoCross$slopes$S.E.[1],
  negEmoCross_minus1DisempSD_A = tmp_negEmoCross$slopes$Est.[3],
  negEmoCross_minus1DisempSD_varA = tmp_negEmoCross$slopes$S.E.[3],
  PFSCross_plus1DisempSD_A = tmp_PFSCross$slopes$Est.[1],
  PFSCross_plus1DisempSD_varA = tmp_PFSCross$slopes$S.E.[1],
  PFSCross_minus1DisempSD_A = tmp_PFSCross$slopes$Est.[3],
  PFSCross_minus1DisempSD_varA = tmp_PFSCross$slopes$S.E.[3],
  posEmoCtry_plus1DisempSD_A = tmp_posEmoCtry$slopes$Est.[1],
  posEmoCtry_plus1DisempSD_varA = tmp_posEmoCtry$slopes$S.E.[1],
  posEmoCtry_minus1DisempSD_A = tmp_posEmoCtry$slopes$Est.[3],
  posEmoCtry_minus1DisempSD_varA = tmp_posEmoCtry$slopes$S.E.[3],
  negEmoCtry_plus1DisempSD_A = tmp_negEmoCtry$slopes$Est.[1],
  negEmoCtry_plus1DisempSD_varA = tmp_negEmoCtry$slopes$S.E.[1],
  negEmoCtry_minus1DisempSD_A = tmp_negEmoCtry$slopes$Est.[3],
  negEmoCtry_minus1DisempSD_varA = tmp_negEmoCtry$slopes$S.E.[3],
  PFSCtry_plus1DisempSD_A = tmp_PFSCtry$slopes$Est.[1],
  PFSCtry_plus1DisempSD_varA = tmp_PFSCtry$slopes$S.E.[1],
  PFSCtry_minus1DisempSD_A = tmp_PFSCtry$slopes$Est.[3],
  PFSCtry_minus1DisempSD_varA = tmp_PFSCtry$slopes$S.E.[3]
)
...
stargazer::stargazer(WB_cw_out, 
                     out="output/tables/WB_CW_relOnly.html",
                     summary=FALSE,
                     type='html',
                     digits=3,
                     digits.extra = 3,
...
Second step: Add all level 1 predictors
```{r}
ana_lv1 <- lme4::lmer(c19ProSo03 ~ negEmo_cw + posEmo_cw + PFS_cw + disemp_cw + 
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
Third step: Add all level 1 predictors and their interactions
```{r}
ana_lv1_int <- lme4::lmer(c19ProSo03 ~ negEmo_cw*disemp_cw + posEmo_cw*disemp_cw + PFS_cw*disemp_cw + 
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
If at all we are underestimating effects on level 2 due to potential small cluster size (Schunck, 2016)
```{r}
ana_lv12 <- lme4::lmer(c19ProSo03 ~ negEmo_cw + posEmo_cw + PFS_cw + disemp_cw +  
                         negEmoPP_cw + posEmoPP_cw + PFSPP_cw + disempPP_cw + 
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
```{r}
ana_lv12_int <- lme4::lmer(c19ProSo03 ~ negEmo_cw*disemp_cw + posEmo_cw*disemp_cw + PFS_cw*disemp_cw + 
                             negEmoPP_cw*disempPP_cw + posEmoPP_cw*disempPP_cw + PFSPP_cw*disempPP_cw +  
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
If at all we are underestimating effects on level 2 due to potential small cluster size (Schunck, 2016)
```{r}
ana_lv123 <- lme4::lmer(c19ProSo03 ~ negEmo_cw + posEmo_cw + PFS_cw + disemp_cw +  
                      negEmoPP_cw + posEmoPP_cw + PFSPP_cw + disempPP_cw + 
                      negEmoCtry_cw + posEmoCtry_cw + PFSCtry_cw + disempCtry_cw + 
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
...
```{r}
ana_lv123_int <- lme4::lmer(c19ProSo03 ~ negEmo_cw*disemp_cw + posEmo_cw*disemp_cw + PFS_cw*disemp_cw +  
                      negEmoPP_cw*disempPP_cw + posEmoPP_cw*disempPP_cw + PFSPP_cw*disempPP_cw + 
                      negEmoCtry_cw*disempCtry_cw + posEmoCtry_cw*disempCtry_cw + PFSCtry_cw*disempCtry_cw +  
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
...
#### cross-level interactions (1,2)
```{r}
ana_lv123_intCrossAll <- lme4::lmer(c19ProSo03 ~ negEmo_cw*disemp_cw + posEmo_cw*disemp_cw + PFS_cw*disemp_cw + 
                             negEmoPP_cw*disempPP_cw + posEmoPP_cw*disempPP_cw + PFSPP_cw*disempPP_cw +  
                             negEmo_cw*disempPP_cw + posEmo_cw*disempPP_cw + PFS_cw*disempPP_cw +  
                             negEmoCtry_cw*disempCtry_cw + posEmoCtry_cw*disempCtry_cw + PFSCtry_cw*disempCtry_cw +
                    (1|coded_country) + # each country can have its own intercept
...
ana_lv123_intCrossOnly <- lme4::lmer(c19ProSo03 ~ negEmo_cw + posEmo_cw + PFS_cw + disemp_cw +
                             negEmoPP_cw + posEmoPP_cw + PFSPP_cw +  disempPP_cw +
                             negEmo_cw*disempPP_cw + posEmo_cw*disempPP_cw + PFS_cw*disempPP_cw +  
                             negEmoCtry_cw + posEmoCtry_cw + PFSCtry_cw + disempCtry_cw +
                    (1|coded_country) + # each country can have its own intercept
...
```{r}
 stargazer::stargazer(ana_lv0, ana_lv1, ana_lv1_int, ana_lv12, ana_lv12_int, ana_lv123, ana_lv123_int, 
                ...
                out="output/tables/proSo_noWellBeing_CW.html")
...
stargazer::stargazer(proSo_noWellBeing_CW_out, 
                     out="output/tables/proSo_noWellBeing_CW_relOnly.html",
                     summary=FALSE,
                     type='html',
                     digits=3,
                     digits.extra = 3,
...
Second step: Add all level 1 predictors
```{r}
ana_lv1 <- lme4::lmer(c19ProSo03 ~ negEmo_cw + posEmo_cw + PFS_cw + disemp_cw + WB_cw +
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
Third step: Add all level 1 predictors and their interactions
```{r}
ana_lv1_int <- lme4::lmer(c19ProSo03 ~ negEmo_cw*disemp_cw + posEmo_cw*disemp_cw + PFS_cw*disemp_cw + WB_cw +
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
If at all we are underestimating effects on level 2 due to potential small cluster size (Schunck, 2016)
```{r}
ana_lv12 <- lme4::lmer(c19ProSo03 ~ negEmo_cw + posEmo_cw + PFS_cw + disemp_cw + WB_cw + 
                         negEmoPP_cw + posEmoPP_cw + PFSPP_cw + disempPP_cw + WBPP_cw +
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
```{r}
ana_lv12_int <- lme4::lmer(c19ProSo03 ~ negEmo_cw*disemp_cw + posEmo_cw*disemp_cw + PFS_cw*disemp_cw + WB_cw*disemp_cw + 
                             negEmoPP_cw*disempPP_cw + posEmoPP_cw*disempPP_cw + PFSPP_cw*disempPP_cw + WBPP_cw*disempPP_cw + 
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
If at all we are underestimating effects on level 2 due to potential small cluster size (Schunck, 2016)
```{r}
ana_lv123 <- lme4::lmer(c19ProSo03 ~ negEmo_cw + posEmo_cw + PFS_cw + disemp_cw + WB_cw + 
                      negEmoPP_cw + posEmoPP_cw + PFSPP_cw + disempPP_cw + WBPP_cw +
                      negEmoCtry_cw + posEmoCtry_cw + PFSCtry_cw + disempCtry_cw + WBCtry_cw +
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
```{r}
ana_lv123_int <- lme4::lmer(c19ProSo03 ~ negEmo_cw*disemp_cw + posEmo_cw*disemp_cw + PFS_cw*disemp_cw + WB_cw*disemp_cw + 
                      negEmoPP_cw*disempPP_cw + posEmoPP_cw*disempPP_cw + PFSPP_cw*disempPP_cw + WBPP_cw*disempPP_cw + 
                      negEmoCtry_cw*disempCtry_cw + posEmoCtry_cw*disempCtry_cw + PFSCtry_cw*disempCtry_cw + WBCtry_cw*disempCtry_cw + 
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)
...
#### cross-level interactions (1,2)
```{r}
ana_lv123_intCrossAll <- lme4::lmer(c19ProSo03 ~ negEmo_cw*disemp_cw + posEmo_cw*disemp_cw + PFS_cw*disemp_cw + WB_cw*disemp_cw +
                             negEmoPP_cw*disempPP_cw + posEmoPP_cw*disempPP_cw + PFSPP_cw*disempPP_cw + WBPP_cw*disempPP_cw + 
                             negEmo_cw*disempPP_cw + posEmo_cw*disempPP_cw + PFS_cw*disempPP_cw + WB_cw*disempPP_cw + 
                             negEmoCtry_cw*disempCtry_cw + posEmoCtry_cw*disempCtry_cw + PFSCtry_cw*disempCtry_cw + WBCtry_cw*disempCtry_cw +
                    (1|coded_country) + # each country can have its own intercept
                    (1|coded_country:PPID), # each person can have its own intercept relative to the country
                  data = dtLong)

ana_lv123_intCrossOnly <- lme4::lmer(c19ProSo03 ~ negEmo_cw + posEmo_cw + PFS_cw + disemp_cw + WB_cw +
                             negEmoPP_cw + posEmoPP_cw + PFSPP_cw +  disempPP_cw + WBPP_cw +
                             negEmo_cw*disempPP_cw + posEmo_cw*disempPP_cw + PFS_cw*disempPP_cw + WB_cw*disempPP_cw +   
                             negEmoCtry_cw + posEmoCtry_cw + PFSCtry_cw + disempCtry_cw + WBCtry_cw +
                    (1|coded_country) + # each country can have its own intercept
...
tmp_PFSPP <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "PFSPP_cw", modx = "disempPP_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_PFSPP
tmp_WBPP <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "WBPP_cw", modx = "disempPP_cw", 
                         johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
tmp_WBPP
# tmp_posEmoCross <- interactions::sim_slopes(ana_lv123_intCrossAll, pred = "posEmo_cw", modx = "disempPP_cw", 
#                          johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
...
```{r}
 stargazer::stargazer(ana_lv0, ana_lv1, ana_lv1_int, ana_lv12, ana_lv12_int, ana_lv123, ana_lv123_int, ana_lv123_intCrossAll, 
                ...
                column.labels = c("randomInt", "level1", "level1+int", "level1-2", "level1-2+int", "level1-3", "level1-3+int"),
                dep.var.labels = c("ProSocial"),
                out="output/tables/proSo_CW.html")
...
# minus 1 SD disemp
  ...
  calc_indirect_effect(
    nameIn = "posEmo_minus1DisempSD",
    pathA = paths_WB$posEmo_minus1DisempSD_A,
    varA = paths_WB$posEmo_minus1DisempSD_varA,
  ...
  calc_indirect_effect(
    nameIn = "posEmoPP_minus1DisempSD",
    pathA = paths_WB$posEmoPP_minus1DisempSD_A,
    varA = paths_WB$posEmoPP_minus1DisempSD_varA,
  ...
  calc_indirect_effect(
    nameIn = "posEmoCross_minus1DisempSD",
    pathA = paths_WB$posEmoCross_minus1DisempSD_A,
    varA = paths_WB$posEmoCross_minus1DisempSD_varA,
    pathB = paths_proSo$WB_B,
    varB = paths_proSo$WB_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # within participant level
  calc_indirect_effect(
    nameIn = "negEmo_minus1DisempSD",
    pathA = paths_WB$negEmo_minus1DisempSD_A,
    varA = paths_WB$negEmo_minus1DisempSD_varA,
    pathB = paths_proSo$WB_B,
    varB = paths_proSo$WB_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # between participant level
  calc_indirect_effect(
    nameIn = "negEmoPP_minus1DisempSD",
    pathA = paths_WB$negEmoPP_minus1DisempSD_A,
    varA = paths_WB$negEmoPP_minus1DisempSD_varA,
    pathB = paths_proSo$WBPP_B,
    varB = paths_proSo$WBPP_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # cross-level (path B stays on the idividual level)
  calc_indirect_effect(
    nameIn = "negEmoCross_minus1DisempSD",
    pathA = paths_WB$negEmoCross_minus1DisempSD_A,
    varA = paths_WB$negEmoCross_minus1DisempSD_varA,
    pathB = paths_proSo$WB_B,
    varB = paths_proSo$WB_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # within participant level
  calc_indirect_effect(
    nameIn = "PFS_minus1DisempSD",
    pathA = paths_WB$PFS_minus1DisempSD_A,
    varA = paths_WB$PFS_minus1DisempSD_varA,
    pathB = paths_proSo$WB_B,
    varB = paths_proSo$WB_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # between participant level
  calc_indirect_effect(
    nameIn = "PFSPP_minus1DisempSD",
    pathA = paths_WB$PFSPP_minus1DisempSD_A,
    varA = paths_WB$PFSPP_minus1DisempSD_varA,
    pathB = paths_proSo$WBPP_B,
    varB = paths_proSo$WBPP_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # cross-level (path B stays on the idividual level)
  calc_indirect_effect(
    nameIn = "PFSCross_minus1DisempSD",
    pathA = paths_WB$PFSCross_minus1DisempSD_A,
    varA = paths_WB$PFSCross_minus1DisempSD_varA,
    pathB = paths_proSo$WB_B,
    varB = paths_proSo$WB_varB,
    covAB = 0,
    rep = 5000, 
...
# plus 1 SD disemp
  ...
  # within participant level
  calc_indirect_effect(
    nameIn = "posEmo_plus1DisempSD",
    pathA = paths_WB$posEmo_plus1DisempSD_A,
    varA = paths_WB$posEmo_plus1DisempSD_varA,
    pathB = paths_proSo$WB_B,
    varB = paths_proSo$WB_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # between participant level
  calc_indirect_effect(
    nameIn = "posEmoPP_plus1DisempSD",
    pathA = paths_WB$posEmoPP_plus1DisempSD_A,
    varA = paths_WB$posEmoPP_plus1DisempSD_varA,
    pathB = paths_proSo$WBPP_B,
    varB = paths_proSo$WBPP_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # cross-level (path B stays on the idividual level)
  calc_indirect_effect(
    nameIn = "posEmoCross_plus1DisempSD",
    pathA = paths_WB$posEmoCross_plus1DisempSD_A,
    varA = paths_WB$posEmoCross_plus1DisempSD_varA,
    pathB = paths_proSo$WB_B,
    varB = paths_proSo$WB_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # within participant level
  calc_indirect_effect(
    nameIn = "negEmo_plus1DisempSD",
    pathA = paths_WB$negEmo_plus1DisempSD_A,
    varA = paths_WB$negEmo_plus1DisempSD_varA,
    pathB = paths_proSo$WB_B,
    varB = paths_proSo$WB_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # between participant level
  calc_indirect_effect(
    nameIn = "negEmoPP_plus1DisempSD",
    pathA = paths_WB$negEmoPP_plus1DisempSD_A,
    varA = paths_WB$negEmoPP_plus1DisempSD_varA,
    pathB = paths_proSo$WBPP_B,
    varB = paths_proSo$WBPP_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # cross-level (path B stays on the idividual level)
  calc_indirect_effect(
    nameIn = "negEmoCross_plus1DisempSD",
    pathA = paths_WB$negEmoCross_plus1DisempSD_A,
    varA = paths_WB$negEmoCross_plus1DisempSD_varA,
    pathB = paths_proSo$WB_B,
    varB = paths_proSo$WB_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # within participant level
  calc_indirect_effect(
    nameIn = "PFS_plus1DisempSD",
    pathA = paths_WB$PFS_plus1DisempSD_A,
    varA = paths_WB$PFS_plus1DisempSD_varA,
    pathB = paths_proSo$WB_B,
    varB = paths_proSo$WB_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # between participant level
  calc_indirect_effect(
    nameIn = "PFSPP_plus1DisempSD",
    pathA = paths_WB$PFSPP_plus1DisempSD_A,
    varA = paths_WB$PFSPP_plus1DisempSD_varA,
    pathB = paths_proSo$WBPP_B,
    varB = paths_proSo$WBPP_varB,
    covAB = 0,
    rep = 5000, 
  ...
  # cross-level (path B stays on the idividual level)
  calc_indirect_effect(
    nameIn = "PFSCross_plus1DisempSD",
    pathA = paths_WB$PFSCross_plus1DisempSD_A,
    varA = paths_WB$PFSCross_plus1DisempSD_varA,
    pathB = paths_proSo$WB_B,
    varB = paths_proSo$WB_varB,
    covAB = 0,
    rep = 5000, 
...
stargazer::stargazer(dt_indirect, 
                     out="output/tables/indirect_effects.html",
                     summary=FALSE,
                     type='html',
                     digits=3,
                     digits.extra = 3,
