...
calc_indirect_effect <- function(pathA, pathB, varA, varB, covAB, rep, conf) {
   pest=c(pathA,pathB)
...
mlm_summary_out <- function(model) {
  ...
  CI = lme4::confint.merMod(model, method = "boot", nsim = 500) 
...
```{r}
# if we want random effects
# remotes::install_github("marklhc/bootmlm")
# library(bootmlm)
# bootmlm::vcov_vc(ana_lv123_intCrossAll, sd_cor = FALSE)


vcov(ana_lv123_intCrossAll)

interactions::sim_slopes(ana_lv123_intCrossAll, pred = "posEmoPP_cw", modx = "disempPP_cw", johnson_neyman = TRUE, jnplot = TRUE, confint = TRUE)
calc_indirect_effect(1, 2, 0.5, 0.8, 0.23, 20000, 95)
