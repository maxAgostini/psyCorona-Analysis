...
tmp <- dtFull %>%
  dplyr::transmute_at(vars(matches(
    paste(c("affAnx", "affCalm", "affDepr", "affEnerg", "affNerv", "affInsp", "affRel",
            "affExh", "PFS01", "happy", "lifeSat", "fail01", "c19ProSo01", "c19ProSo03"), collapse = "|"), -contains("DO"))), #vars changed
    as.numeric)
tmp$PPID <- dtFull$PPID
...
dtLong <- data.table::melt.data.table(
  data = tmp, 
  id.vars = "PPID",
  measure.vars = patterns( # cannot access patterns directly! need to load library!!!
    affAnx = "affAnx",
    affCalm = "affCalm",
    affDepr = "affDepr",
    affEnerg = "affEnerg",
...
dtLong <- bmlm::isolate(tmp1, by = "id",
