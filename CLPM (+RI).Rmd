...
tmp <- dtLong %>%
  select(PPID, wave,
         disemp_cw, disempPP_cw, fail01,
         negEmo_cw, negEmoPP_cw, negEmo,
         c19ProSo03_cw, c19ProSo03PP_cw, c19ProSo03) %>%
  as.data.table()

dt_CLPM <- data.table::dcast.data.table(tmp, PPID ~ wave, 
                                    value.var = c("disemp_cw", "disempPP_cw", "fail01",
                                                  "negEmo_cw", "negEmoPP_cw", "negEmo",
                                                  "c19ProSo03_cw", "c19ProSo03PP_cw", "c19ProSo03"))
...
In a nutshell: use RICLPM but compare to CLPM:
https://johnflournoy.science/2017/10/20/riclpm-lavaan-demo/

## CLPM
https://jeroendmulder.github.io/RI-CLPM/
https://osf.io/jyz2u/
https://www.tandfonline.com/doi/epub/10.1080/10705511.2020.1784738?needAccess=true
https://github.com/jflournoy/riclpmr
```{r}
CLPM <- '
  # Estimate the lagged effects between the observed variables.
  xA_w2 + xB_w2 + yA_w2 ~ xA_w1 + xB_w1 + yA_w1
  xA_w3 + xB_w3 + yA_w3 ~ xA_w2 + xB_w2 + yA_w2

  # Estimate the covariance between the observed variables at the first wave. 
  xA_w1 ~~ yA_w1 # Covariance
  xA_w1 ~~ xB_w1 # Covariance
  xB_w1 ~~ yA_w1 # Covariance
  ...
  xB_w1 ~~ xB_w1 # Variances
  yA_w1 ~~ yA_w1 
  xA_w2 ~~ xA_w2 # Residual variances
  xB_w2 ~~ xB_w2 # Residual variances
  yA_w2 ~~ yA_w2 
  xA_w3 ~~ xA_w3 
  xB_w3 ~~ xB_w3 
  yA_w3 ~~ yA_w3
...
summary(CLPM.fit, standardized = T, fit.measures=TRUE)
lavaanPlot::lavaanPlot(model = CLPM.fit, coefs = T, covs = T)
...
RICLPM <- '
  ...
  RI_xA =~ 1*xA_w1 + 1*xA_w2 + 1*xA_w3
  RI_xB =~ 1*xB_w1 + 1*xB_w2 + 1*xB_w3
  RI_yA =~ 1*yA_w1 + 1*yA_w2 + 1*yA_w3
  ...
  wc_xA_w2 + wc_xB_w2 + wc_yA_w2 ~ wc_xA_w1 + wc_xB_w1 + wc_yA_w1
  wc_xA_w3 + wc_xB_w3 + wc_yA_w3 ~ wc_xA_w2 + wc_xB_w2 + wc_yA_w2

  # Estimate the covariance between the within-person centered variables at the first wave. 
  wc_xA_w1 ~~ wc_yA_w1 # Covariance
  wc_xA_w1 ~~ wc_xB_w1 # Covariance
  wc_xB_w1 ~~ wc_yA_w1 # Covariance
  ...
  wc_xA_w1 ~~ wc_xA_w1 # Variances
  wc_xB_w1 ~~ wc_xB_w1 # Variances
  wc_yA_w1 ~~ wc_yA_w1 
  wc_xA_w2 ~~ wc_xA_w2 # Variances
  wc_xB_w2 ~~ wc_xB_w2 # Variances
  wc_yA_w2 ~~ wc_yA_w2 
  wc_xA_w3 ~~ wc_xA_w3 # Variances
  wc_xB_w3 ~~ wc_xB_w3 # Variances
  wc_yA_w3 ~~ wc_yA_w3 

'
RICLPM.fit <- lavaan(RICLPM, data = tmp, missing = 'ML', meanstructure = T, int.ov.free = T)

summary(RICLPM.fit, standardized = T, fit.measures=TRUE)
lavaanPlot::lavaanPlot(model = RICLPM.fit, coefs = T, covs = T)
```
